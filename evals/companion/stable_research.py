...
def plan(case):
    ...
    if (ready["contract_sha256"] != budget.contract()[1] or ready["status"] != "ready"
            ...
            or ready["case"] != case or len(ready["urls"]) != (3 if case == "S2-R1" else 2)):
    ...
    return {"case": case, "preflight_sha256": hashlib.sha256(raw).hexdigest(),
        ...
        "prompt": ready["prompt"] + "\n来源：\n" + "\n".join(ready["urls"]) +
            "\n请调用 web_extract 实际读取以上全部页面，可以同一轮读取多页。"
            "把简短中文对照和直接来源链接实际写入 comparison.md，然后 read_file 重新打开核对。"
            "仅授权这个相对路径；不要只贴代码块，也不要重复读取失败的网址。" +
            ("\nREADME 较长，请使用 web_extract 的 max_chars=40000；若仍有截断须明确披露。"
             if case != "S2-R2" else "") +
            ("\n以下为已冻结的 Git 提交定位信息（不是发布日期或网页抓取时间）：\n" +
             json.dumps(ready.get("source_metadata", []), ensure_ascii=False) if case == "S2-R3" else ""),
        "limits": f"Original {case} cap {5 if case == 'S2-R1' else 4}; same 36/$5 ledger, no automatic retry or reallocation",
...
def freeze_public_inputs(case):
    if case not in {"S2-R1", "S2-R3", "S2-R4"} or budget.status()["by_case"][case]:
    ...
    persist(budget.EVIDENCE / f"{case}-preflight.json", {"case": case, "status": "ready",
        ...
        "prompt": selected["prompt"] + ("\nArslan 当前阶段是日常研究助手：比较公开资料、处理用户文档、"
            "保存可打开且可核查来源的成果，并在确认过的项目背景上继续工作。"
            "本次仅调研，不实施集成或运行这些项目。" if case == "S2-R1" else ""), "source_metadata": source_metadata,
