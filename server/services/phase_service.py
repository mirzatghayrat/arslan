...
async def set_clarifying(conversation_id: str) -> None:
    """Pin a 'clarifying' create phase for the conversation (one pending row,
    same upsert semantics as set_proposing). While active, the turn model
    suppresses routing to existing spawns so the follow-up keeps clarifying."""
    async with db_session.AsyncSessionLocal() as db:
        await db.execute(
            delete(SpawnPhase).where(SpawnPhase.conversation_id == conversation_id)
        )
