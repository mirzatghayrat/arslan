"""Optimizer: propose bounded section edits to a spawn's instruction document from
replay evidence.

One LLM call. On any failure or empty output, returns [] (a no-op round), so the
evaluator will find no improvement and the gate will not promote.
"""
