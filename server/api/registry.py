...
from server.registry.service import is_assignable
    ...
    return RegistryOut(
        toolsets=[
            ToolsetOut(
                ...
                status=t.status, assignable=is_assignable(t.tier, t.status),
        ...
        skills=[
            SkillPackOut(
                ...
                tier=s.tier, status=s.status, assignable=is_assignable(s.tier, s.status),
            )
            for s in skills
        ],
    )
