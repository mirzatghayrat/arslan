...
### ① 开机自动 rekey,还是挂在一个闸后面? → 🟢 **B(人闸)**

§2.2 的 rekey **写用户的数据**(重写密文列)。虽然只在**成功解密之后**才写、写完立即读回校验,
它仍然是启动时对用户数据的自动写。

- **A(我原倾向,已被否)**:自动做,写后校验,失败回滚 + 大声,UI 报「已恢复 N 条」。
- 🟢 **B(裁决)**:**只诊断 + 只读试解,报告能不能恢复;rekey 挂在明确的人闸后面。**

🔴 **我对裁决的读法(⚠️ 待用户确认,不阻塞开工)**:用户的原话是
「恢复走『只读试解 → 报告 → 我们点头才 rekey』—— 批,**一个字节都不许在点头前写**」。
那句话字面上直接排除 A(自动写),所以我把它读成**同时**裁决了
拍板项①(产品行为)和拍板项②(本轮拿真库验收的手法)——
**不是**只裁决后者。若原意只指本轮验收、产品仍要自动 rekey,回来改这一条。

**这条落到实现上的硬约束**:诊断路径、试解路径**必须是纯只读的**,
不许有任何「顺手修一下」的写。这一点要有测试钉住(§4.4 新增第 8 条)。
...
8. 🔴 **诊断与试解路径是纯只读的**(裁决 ①B:「一个字节都不许在点头前写」)。
   断言形状:在诊断/试解全程用一个**会对任何写操作抛异常**的 session(或对
