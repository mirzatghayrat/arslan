...
**读**的时候,`MultiFernet` 的钥匙串从 2 把扩成 N 把,顺序固定:

```
[ PBKDF2(secret, DB 盐)          ← 主钥匙,写只用这一把
  PBKDF2(secret, <data_dir>/crypto_salt 文件里的盐)   ← 若存在且与 DB 行不同
  PBKDF2(secret, _FALLBACK_SALT) ← 历史上被静默用过的那把
  SHA256(secret)                 ← legacy 无盐,已有
]
```

🔴 **并且:非主钥匙命中时立刻用主钥匙重加密回去(rekey-on-read)。**
这是 `d6d8afa8` 跳过的那一步 —— 只做读时回退不做重加密,就是把「下一次钥匙串再变一次
就永久失去」这颗雷留在原地。重加密后**立即用主钥匙读回校验**再提交;失败则回滚并大声。

覆盖面:`_SECRET_KEYS` 三个 + `ProviderConfig.api_key` + `MCPServer.env`。
一个共享的 `crypto.rekey_if_stale(enc) -> (plaintext, new_enc | None)`,四个消费者共用。
...
### ① 开机自动 rekey,还是挂在一个闸后面?

§2.2 的 rekey **写用户的数据**(重写密文列)。虽然只在**成功解密之后**才写、写完立即读回校验,
它仍然是启动时对用户数据的自动写。

- **A(我倾向)**:自动做,写前不备份、写后校验,失败回滚 + 大声,并在 UI 报「已恢复 N 条」。
  理由:不自动做等于把恢复藏在一个用户不知道存在的按钮后面 ——
  而这个缺陷的全部特征就是**用户不知道发生了什么**。
- **B**:只诊断,给一个明确的「恢复我的密钥」按钮。更保守,但多一轮往返,
  且大多数人不会点一个他们看不懂的按钮。
...
- **A(我倾向)**:**只读试解**。写一个一次性脚本,只报告「能/不能解开、命中的是哪一把」,
  **一个字节都不写**。拿到「能」这个事实,再由你点头是否真 rekey。
- **B**:直接在真库上跑完整 rekey。
  否掉的理由:那是 [[arslan-user-data-backup]] 里那个不可再生的库,而本轮的代码
  在那一刻还一次都没在真数据上跑过。

⚠️ 无论 A 还是 B,动之前都要按 `backup-data.sh` 走一遍备份,且
**`~/arslan-backups/` 一个字节都不碰**。

### ③ 三个 `*_status` 字段进 `GET /settings`,还是单开一个只读端点?

- **A(我倾向)**:进 `GET /settings`,和 provider 那边的 `key_status` 形状对齐。
  一次往返、一个心智模型。
- **B**:单开 `GET /settings/secret-state`。好处是 §0.7 那条注册表纪律完全不用碰。
