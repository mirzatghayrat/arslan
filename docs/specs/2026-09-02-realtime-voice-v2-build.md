...
> 状态:**已批(2026-09-02)**。用户对 §9 三条拍板全按建议:V2a 半双工先出 · V2 打断不取消生成 · `voice_mode` 默认 `push_to_talk`。决策①仍由探针 P1 决定。§0.2 的红灯已随 PR #82 修入 main。上位 spec = `2026-09-02-realtime-voice.md`(三条建议已同意,V1a 已随 v0.1.36 出货,V1b 在 PR #81)。
...
### 3.1 端点检测(什么时候算你说完)—— Rust 纯函数
输入:识别器的 partial 流(文本 + 到达时刻)。规则(V2 用静音超时,上位 spec 定的):
- partial 非空,且 `now - last_partial_change ≥ endpoint_silence_ms`(默认 900,可调)⇒ **端点** ⇒ 壳发 `{"c":"end_utterance"}` → helper `request.endAudio()` → 识别器出 final → **立刻重新 arm 一个新 request**(常开的关键:识别 session 是一句一个,不是永远一个)。
...
### 3.3 AEC(V2b)
`AVAudioEngine.inputNode.setVoiceProcessingEnabled(true)`(macOS 10.15+)。它顺带做 AGC 和降噪,格式由它定(P3 量它的采样率和 SFSpeech 是否接受)。**A 方案**:TTS 通过 `AVSpeechSynthesizer.write(_:toBufferCallback:)` 拿 PCM,`AVAudioPlayerNode` 挂到同一 engine 的 mainMixer 播放 —— 参考信号天然在。**B 方案**:什么都不接,靠系统。**哪个成立 P1 说了算。**
...
| # | 问题 | 结果决定什么 |
|---|---|---|
| **P1** | VP 开着、webview 的 Web Speech 放一句话、麦克风同时喂 SFSpeech:转写里**有没有那句话**?再用 engine 内 playerNode 放同一句,对照 | 决策①(A/B);barge-in 阈值的真实残余量 |
| **P2** | 常开下 `endAudio` → final → 新 request 重新 arm 的间隔多长;on-device 连续用有没有时长上限;重启时有没有重复 final | 3.1 的重 arm 策略和去重规则 |
| **P3** | `setVoiceProcessingEnabled(true)` 后 inputNode 格式是什么;SFSpeech 收不收 | 3.3 要不要 AVAudioConverter |
...
- **尚无证据**:P1–P4 全部;VP 的 AGC/降噪对中文识别率的影响;常开一小时的 CPU/电量(只能真机量);「AEC 不完美时两个字阈值够不够」。
