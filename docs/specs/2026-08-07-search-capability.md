...
**状态**:🟢 **已批(用户 2026-08-08)。范围不缩:(a) 零 key 兜底 + (b) 三态可见,本轮都要。
仅 §3③(乙 Brave 的排期)仍开着。**
...
> - **拍板项① 裁决 (c):(a) 和 (b) 都要** —— 我把它设成二选一,前提本身就错了。详见 §3①。
> - **仍开着**:§3 拍板项③(乙 Brave 这轮做还是后置一轮)。
...
## 3. 拍板项 —— **①(c) 两个都要 · ② 自己实现 · ③ 仍开着(乙的排期)**

### ① 真实抱怨是「要付费」还是「免费额度用完」? → 🟢 **(c):(a) 和 (b) 都要**
...
**用户 2026-08-08 裁决:(c) —— 两个都要,不是二选一。** 原话:

> **(a) 裸装不配任何 key 也能搜** —— 「下载就能用」是产品承诺,零 key 兜底是核心交付;
> **(b) 配了 key 之后,坏了/额度用完了必须大声** —— 「我自己就是被『钥匙碎了却报没配置』
> 坑了一个月的人,三态可见、每次搜索标明由哪个 provider 服务,是底线。」

⇒ 我提这个拍板项时把它设成了二选一,**那个前提本身是错的**:我把「抱怨从哪来」
当成了「重心放哪」,而两条其实是同一个缺陷的两端 —— **(a) 是没有钥匙时不该死,
(b) 是有钥匙但钥匙坏了时不该谎报成没钥匙。** 用户是后者的当事人。

**落到范围上**:
- 「甲乙都做」的原裁决**维持,范围不缩**。
- **本轮 (a)(b) 必须都在。** (a) = §2.2 的零 key 兜底 provider;
  (b) = §2.6 的语义分类 + 退避 + `toolHumanize` 不吞状态码,**加上**
  §2.4 的出处标注(每次搜索标明由哪个 provider 服务)。
- **乙(Brave)为工期可以后置一轮**(见 §3③),但那是**乙**的排期,不是 (b) 的。
- 🔴 **(b) 与 spec ⓪ 的第三态是同一件事的两半**:⓪ 负责「设了但解不开」这个**存储态**,
  ① 负责「设了但连不通 / 额度尽 / key 被拒」这个**运行态**。
  用户被坑的那一个月正是两者被压成同一句 `"no API key set"`。见 §6.1 接缝。
- `SearchToolsSection.tsx:113-115` 那句死英文照修(与哪个答案无关)。
