...
## 4b. 用户裁决(2026-08-20,已定)

1. **顺序**:P1 → P2 → P3(同意建议:地基先,最大即时跃迁)。
2. **A 粒度**:T1 = 首次授权 + session 内免确认。不再松不再紧。
3. **C4 免执行闸**:**不开**。已登记 node 的远程 T2 永远逐条确认——那个开关会把 arXiv 那篇的「持久化 + 无人执行」凑齐,是产品性质的改变。
4. **节奏**:随做随发小版本,P1 好了就发;「v0.2.0 长出手脚」的旗号留到 P3 完成时打。

**P1 施工 spec**:`2026-08-20-p1-local-execution-surface.md`(总纲的 file:line 已被其 §0 的重核结论顶替)。

## 5. 待用户裁决点(原始,已由 §4b 全部裁决)
