...
### P3a — LAN 发现(只读,提议面) ✅ **已出货(#54 `3a59255a`)**
...
### P3b — SSH 外联 + 远程执行(执行面,per-host 人闸) → 施工 spec:`2026-08-21-p3b-ssh-reach.md`
...
- 传输:sidecar `ssh` 子进程(复用 command_sandbox 的 seatbelt 包裹),**不引入 paramiko/asyncssh**(少一个依赖面)。
  - ⚠️ **2026-08-21 更正**:原文这里写「系统 ssh 的 known_hosts 语义是用户熟悉的」——**错的**。`command_sandbox` 故意把 HOME 刷成临时目录,ssh 读不到 `~/.ssh`;Arslan 会维护自己的 known_hosts。详见 P3b spec §0.4。
...
> ②③④ 的**建议答案 + 新证据**见 `2026-08-21-p3b-ssh-reach.md` §1;③ 在那里被补了一条本 spec 没考虑到的防线(远端有 shell,本地没有)。

1. ~~**P3a 单独先发**还是三阶攒一起发?~~ **已定并已执行**:P3a 单独出货(#54 `3a59255a`,随 v0.1.27 出)。P3b/c 攒一起。
