...
- **The data directory is the unit of truth and the unit of backup.** The DB, your notes, the encrypted secrets, `crypto_salt`, and `api_token` all live under `ARSLAN_DATA_DIR` (or the platform app-data dir). Back it up / restore it **as a whole** — losing `crypto_salt` makes PBKDF2-encrypted BYOK secrets undecryptable even with the right `ARSLAN_SECRET_KEY`.
...
- **Migrations are hand-wired at boot, not `alembic upgrade`.** Each schema change is a `server/db/migrations/versions/_00NN_name.py` file exposing an **idempotent** `upgrade_sync(connection)` (guarded with `PRAGMA table_info(...)` + `exec_driver_sql`). It is then wired into the boot chain in `server/main.py`'s `lifespan()` — `Base.metadata.create_all` first, then each `upgrade_sync` in sequence via `await conn.run_sync(...)`. Adding the file **without** wiring it into `main.py` means it never runs. (An `alembic.ini` + `migrations/env.py` scaffold also exists, but the operative boot mechanism is this hand-wired chain — see [CONTRIBUTING.md](../CONTRIBUTING.md#hand-wired-migrations).)
