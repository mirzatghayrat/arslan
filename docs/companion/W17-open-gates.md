...
| --- | --- | --- |
| Safe user-facing recovery (I01/I05, W17) | Native cancellation/activation/restart, pause/bound rollback and wrong-key checks pass with retained data/credentials. Restore-time language hints and splash refresh fix the observed language defects; six pending-recovery language sheets/notices and Chinese-to-Japanese rollback are now clicked/observed. | Remaining filesystem/timeout/error native branches; dedicated maintenance view; full native layout acceptance; custom durable key configuration. |
