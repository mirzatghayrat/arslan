// Vendored from reactbits.dev (DavidHDev/react-bits) Animations/Strands.
// Source: https://raw.githubusercontent.com/DavidHDev/react-bits/main/src/content/Animations/Strands/Strands.jsx
// Ported to TSX; shaders/uniforms/animation kept identical. Adapted to this repo's
// conventions (see Prism.tsx): ResizeObserver instead of a window "resize" listener,
// inline canvas sizing (no separate CSS file), and a jsdom-safe early return since
// jsdom has no WebGL context (renderer.gl is undefined there).
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    // jsdom (and other non-GPU test/SSR environments) has no WebGL context: a
    // canvas's getContext('webgl2'/'webgl') returns null there. ogl's Renderer
    // constructor unconditionally does `this.gl.renderer = this` right after
    // requesting the context, which throws on a null gl instead of failing
    // gracefully. So we must probe for WebGL support *before* constructing the
    // Renderer at all — this component is mounted indirectly via BrainOrbCore
    // inside KnowledgeSunburst's (unmocked) jsdom tests, so this guard is load
