  ...
  return <section className="space-y-3 rounded-lg border border-border p-3" aria-label={t("validation.title")}>
    ...
    {!!report.artifacts.length && <div className="space-y-2 border-t border-border pt-3">
      ...
      {report.artifacts.map(artifact => <div key={artifact.id} className="space-y-1 text-xs">
        <p className="break-all">{artifact.title || artifact.filename || t("validation.artifacts")}</p>
        <p className={artifact.status === "failed" ? "text-destructive" : "text-muted-foreground"}>{status(artifact.status)}</p>
      </div>)}
    </div>}
    <details className="text-xs text-muted-foreground"><summary className="cursor-pointer">{t("validation.details")}</summary>
