...
export default function ConnectionsSection({ prefill, onOpenSettings, provider }: {
  ...
  useEffect(() => {
    ...
    request<{ local: { generate: boolean; edit: boolean }; blocked_reason: string }>("/connections/local-media/capabilities")
      .then(value => { if (alive) setMediaUnavailable(value.local?.generate === false && value.local?.edit === false && value.blocked_reason === "media_host_setup_required"); })
      .catch(() => { if (alive) setMediaUnavailable(false); });
    return () => { alive = false; };
  }, []);
  return <div className="h-full overflow-y-auto p-5 lg:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-xl border border-border p-4"><h2 className="font-semibold">{t("design.mediaTitle")}</h2>
      <p className="mt-2 text-sm text-muted-foreground">{t(mediaUnavailable ? "design.mediaUnavailable" : "workspace.connectionStatusUnknown")}</p>
      <p className="mt-2 text-sm text-muted-foreground">{t("design.mediaFallback")}</p>
    </section>
