  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <label className="block text-[10.5px] font-sans font-medium text-muted-foreground ">
          {t("settings.palette")}
        </label>
        <div role="radiogroup" aria-label={t("settings.palette")} className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {PALETTES.map((p) => (
            <button
              key={p.id}
              role="radio"
              ...
              className={`flex min-w-0 items-center gap-2 rounded-xl border px-3 py-2 text-xs transition-all ${
                palette === p.id ? "border-primary ring-1 ring-ring" : "border-border hover:border-border-strong"
              }`}
            >
              <span className="flex shrink-0 h-5 w-5 items-center justify-center rounded-md border border-border" style={{ background: mode === "light" ? p.swatch.bgLight : p.swatch.bg }}>
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: p.swatch.accent }} />
              </span>
              <span className="min-w-0 text-left leading-snug text-foreground break-words">{t(p.nameKey)}</span>
            </button>
          ))}
        </div>
      </div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div role="radiogroup" aria-label={t("settings.mode")} className="flex flex-wrap gap-1">
