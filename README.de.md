...
| <img src="docs/assets/icons/graduation-cap.svg" width="20"><br/>**Selbstevolution mit Prüfungs-Gate** | Der Prompt eines Spawns überarbeitet sich selbst — aus der eigenen Run-Historie — und tritt dann auf zurückgehaltenen früheren Aufgaben gegen den Amtsinhaber an, blind bewertet mit vertauschten Positionen. Er muss **mindestens 60 % von mindestens 10 Holdout-Paaren** gewinnen, und **keine Dimension** (fabrication, identity, completion) darf **schlechter** abschneiden als der Amtsinhaber. Bestanden → ein lesbarer Diff landet in deiner Inbox. **Nichts wird wirksam, bevor du auf Promote drückst.** |
| <img src="docs/assets/icons/shield-check.svg" width="20"><br/>**Sicher per Default, nicht per Disclaimer** | Generierter Code läuft ohne Netzwerkzugriff in einer macOS-Kernel-Sandbox; fehlt diese, wird die Ausführung verweigert. Ausgewählte Netzwerkbefehle nutzen eingeschränkten öffentlichen Transport ohne Anmeldung. Zugangsdatenabhängige Repository- und Kontoaktionen bleiben bis zur Abnahme eines isolierten Brokers deaktiviert. |
| <img src="docs/assets/icons/brain.svg" width="20"><br/>**Ein Second Brain, das du durchsehen und korrigieren kannst** | Materialien, Learnings, ein Profil und `[[wiki-link]]`-Notizen — hybrides Retrieval aus FTS5 + Embeddings, erkundbar als kraftbasierter Graph im Obsidian-Stil. Einträge halten fest, wann sie wirksam wurden und was sie abgelöst hat, und der Graph lässt sich **nach Wirksamkeitszeitpunkt filtern** — ein Filter über noch vorhandene Einträge, keine Wiedergabe der Vergangenheit. |
| <img src="docs/assets/icons/badge-check.svg" width="20"><br/>**Ehrlich by Design** | Guardrails fangen erfundene „Hab ich schon erledigt“-Behauptungen ab und halten die Selbstauskunft des Agents an das gebunden, was tatsächlich lief. Ein **Löschen** greift nie von selbst — es landet in einer Inbox, die du annimmst oder verwirfst. Ein **Überschreiben** durch den Host-Agenten greift sofort, schreibt aber einen Zeiger statt den Text zu ersetzen: der ursprüngliche Eintrag bleibt, ein Klick macht es rückgängig. Was ein Spawn am gemeinsamen Gedächtnis vorschlägt, geht ebenfalls in die Inbox. |
| <img src="docs/assets/icons/key-round.svg" width="20"><br/>**Local-first, Bring Your Own Key** | Dein Rechner, deine API-Keys — und **null Drittanbieter-Server** dazwischen. Mit mehreren konfigurierten Providern kannst du Routing über sie einschalten (standardmäßig ein Modell); Judge- und Router-Rollen bleiben an dein Primärmodell gebunden, damit die Bewertung nie auf ein billigeres abdriftet. Kommt mit i18n in 6 Sprachen und 6 Theme-Paletten (hell + dunkel). |
...
Der Prompt eines Spawns wird automatisch überarbeitet — und muss sich dann erst auf zurückgehaltenen früheren Aufgaben beweisen, bevor du ihn überhaupt zu Gesicht bekommst: mindestens 10 Paare ohne Unentschieden, davon mindestens 60 % gewonnen, und keine Dimension schlechter als der Amtsinhaber. Siege auf synthetischen Aufgaben können einen Rückschritt auf echten nicht ausgleichen, und ein Kandidat kann nicht gewinnen, indem er länger wird. Durchgefallen → verworfen, taucht nie auf. Bestanden → eine Vorschlagskarte mit lesbarem Diff; die Änderung greift **erst, wenn du auf Promote klickst**.

## Ein Second Brain, filterbar nach Wirksamkeitszeitpunkt

<div align="center">
  <img src="docs/assets/fig03-second-brain.png" alt="ABB. 03 — Second Brain: Gedächtnis bildet sich automatisch, Spawns lesen es per hybridem Retrieval, vom Modell vorgeschlagene Löschungen laufen durch deine Inbox, und Einträge halten fest, wann sie wirksam wurden" width="100%">
</div>

Das Gedächtnis bildet sich von selbst — vom Router extrahierte Fakten und Destillation am Session-Ende — und Spawns lesen es per hybridem FTS5-+-Embedding-Retrieval zurück. Einträge halten fest, wann sie wirksam wurden und was sie abgelöst hat, und den Graphen im Obsidian-Stil kannst du nach Wirksamkeitszeitpunkt filtern. Genau gesagt: ein Filter über Einträge, die es noch gibt — keine Wiedergabe der Vergangenheit. Löschungen und In-Place-Änderungen hinterlassen keine Spur, und das Ende eines Eintrags wird aus seinem Nachfolger abgeleitet, nicht aufgezeichnet. Will das Modell etwas **löschen**, landet der Vorschlag zuerst in deiner Inbox. Ein **Überschreiben** durch den Host-Agenten greift sofort, aber der abgelöste Eintrag bleibt erhalten und ein Klick stellt ihn wieder her.
