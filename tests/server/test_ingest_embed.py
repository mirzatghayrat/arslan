...
async def test_ingest_into_collection(maker):
    from server.services import ingest
    n = await ingest.ingest_text(None, "a.txt", "共享资料内容", collection_id=9)
    assert n == 1
...
async def test_ingest_requires_exactly_one_target(maker):
    from server.services import ingest
    with pytest.raises(ValueError):
        await ingest.ingest_text(1, "a", "x", collection_id=9)
    with pytest.raises(ValueError):
        await ingest.ingest_text(None, "a", "x")


async def test_ingest_embeds_when_provider_available(maker, monkeypatch):
    ...
    n = await ingest.ingest_text(1, "b.txt", long_text)
    assert n > 1
...
async def test_ingest_survives_embed_failure(maker, monkeypatch):
    ...
    async def _fake_active():
        return Boom()
    monkeypatch.setattr(embedding_service, "active_provider", _fake_active)
    n = await ingest.ingest_text(1, "c.txt", "内容")
    assert n == 1
    async with maker() as s:
        result = (await s.execute(sa_text(
