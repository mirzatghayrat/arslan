...
def test_generate_skill_draft_readonly(maker, monkeypatch):
...
def test_create_skill_persists_safe_registered(maker):
    ...
    async def _run():
        ...
        return created, row
    created, row = anyio.run(_run)
    assert created["key"].startswith("gh-")
    assert row.tier == "safe" and row.status == "registered"
    assert row.body == body and row.name == "My Tech"


def test_create_skill_missing_sections_raises(maker):
    from server.services import discovery_service as ds
    with pytest.raises(ValueError):
        anyio.run(lambda: ds.create_skill("o/r", "n", "c", "d", "no sections here"))


def test_create_skill_upserts_by_key(maker):
    from server.services import discovery_service as ds
    body = "## Trigger\nx\n## 决策规则\ny"
    async def _run():
