...
def test_complete_chat_distills_then_archives(maker, monkeypatch):
    ...
    async def _run():
        ...
        return n, len(active), len(archived)
    n, active, archived = anyio.run(_run)
    assert captured["spawn_id"] == 1
    assert "把开头改紧凑" in captured["signals"]
    assert n == 2 and active == 0 and archived == 2


def test_complete_chat_empty_is_noop(maker, monkeypatch):
    from server.services import chat_lifecycle, distill_service
    called = {"n": 0}
    async def fake_distill(spawn_id, signals):
        called["n"] += 1
    ...
    async def _run():
        ...
        return await chat_lifecycle.complete_chat(1)
    second = anyio.run(_run)
    assert second == 0
