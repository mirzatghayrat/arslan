...
def test_exception_path_persists_and_enriches(maker, monkeypatch):
...
def test_iserror_path_persists_and_enriches(maker, monkeypatch):
    from server.mcp import session
...
def test_stored_error_truncated_to_500(maker, monkeypatch):
    from server.mcp import session
    big = "x" * 2000
...
def test_success_clears_last_error(maker, monkeypatch):
    from server.mcp import session
    _set_seed_error(maker, "old failure")
    async def fake_call(server, name, args): return _CallResult([_Text("body")])
    monkeypatch.setattr(session.manager, "call_tool", fake_call)

    res = _execute()
    assert res["ok"] is True and "body" in res["summary"]
    assert _last_error(maker) is None


def test_persistence_failure_does_not_mask_error(maker, monkeypatch):
    from server.mcp import session
...
def test_error_field_goes_through_wrap_external(maker, monkeypatch):
