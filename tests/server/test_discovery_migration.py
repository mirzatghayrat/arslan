...
def test_migration_0012_creates_table(tmp_path):
    ...
    assert "discovery_candidates" in anyio.run(_run)


def test_candidate_model_roundtrip(tmp_path):
    ...
    async def _run():
        ...
        async with m() as s:
            ...
            return row.full_name, row.snapshot
    name, snap = anyio.run(_run)
