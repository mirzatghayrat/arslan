...
async def test_build_spawn_system_includes_equipment_and_guidance(maker):
    from server.orchestrator.dispatcher import build_spawn_system
    async with maker() as s:
        spawn = await s.get(Spawn, 7)
    out = await build_spawn_system(spawn, retrieval_query="do X", current_turn=1)
    system, wired = out
...
async def test_build_spawn_system_has_language_directive(maker):
    ...
    async with maker() as s:
        spawn = await s.get(Spawn, 7)
    system, _ = await build_spawn_system(spawn, retrieval_query="x", current_turn=1)
    low = system.lower()
...
async def test_build_spawn_system_has_html_delivery_guardrail(maker):
...
async def test_build_spawn_system_override(maker):
    from server.orchestrator.dispatcher import build_spawn_system
    async with maker() as s:
        spawn = await s.get(Spawn, 7)
    system, _ = await build_spawn_system(spawn, retrieval_query="x", current_turn=1, system_prompt_override="OVERRIDE PERSONA")
