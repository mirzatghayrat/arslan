...
async def test_migration_0010_creates_mcp_servers_and_external_name(tmp_path):
...
async def test_mcpserver_model_roundtrips(tmp_path):
    ...
    async with maker() as s:
        ...
        label, command, args, status = row.label, row.command, row.args, row.status
