...
async def test_embed_missing_backfills_null_and_stale(maker, monkeypatch):
    ...
    done = await es.embed_missing(batch_size=2)
...
async def test_embed_missing_noop_without_provider(maker):
    from server.services import embedding_service as es
    assert await es.embed_missing() == 0


async def test_embed_missing_short_vector_response_terminates(maker, monkeypatch):
...
async def test_embed_missing_partial_progress_honest_on_later_batch_failure(maker, monkeypatch):
