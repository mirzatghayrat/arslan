...
async def test_active_provider_none_when_unconfigured(maker):
    from server.services import embedding_service as es
    assert await es.active_provider() is None


async def test_active_provider_preference_order(maker, monkeypatch):
    """zhipu 配了就选 zhipu embedding-3。"""
    from server.services import embedding_service as es
    from server import crypto
    from server.db.models import ProviderConfig
    async with maker() as s:
        s.add(ProviderConfig(label="t", provider="zhipu", model="x",
                             api_key=crypto.encrypt("sk-test-12345678"), is_primary=True))
        await s.commit()
    p = await es.active_provider()
    assert p is not None and p.model_id == "embedding-3"
    assert "bigmodel.cn" in p.base_url


async def test_active_provider_skips_non_embedding_provider(maker):
    from server.services import embedding_service as es
    from server import crypto
    from server.db.models import ProviderConfig
    async with maker() as s:
...
async def test_embed_reorders_out_of_order_response(monkeypatch):
