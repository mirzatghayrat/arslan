...
| <img src="docs/assets/icons/graduation-cap.svg" width="20"><br/>**試験ゲート付きの自己進化** | スポーンのプロンプトは、自身の実行履歴から自らを書き換えます — そのうえで、ホールドアウトされた過去タスクで現行版と対戦し、左右を入れ替えたブラインド評価にかけられます。**最低 10 組のホールドアウト対戦で 60% 以上に勝ち**、かつ **どの評価軸(fabrication / identity / completion)でも現行版より悪くない**ことが条件です。合格すれば、読みやすい差分が受信箱に届きます。**あなたが Promote を押すまで、何も反映されません。** |
| <img src="docs/assets/icons/shield-check.svg" width="20"><br/>**免責ではなく、デフォルトで安全** | 生成コードは macOS のカーネルサンドボックス内でネットワークを遮断して実行され、利用できない場合は実行を拒否します。指定のネットワークコマンドは制限された公開・未認証通信のみ利用できます。認証情報が必要なリポジトリやアカウント操作は、隔離ブローカーと安全性の検証が完了するまで無効です。 |
| <img src="docs/assets/icons/brain.svg" width="20"><br/>**見て直せるセカンドブレイン** | 資料、学び、プロフィール、そして `[[wiki-link]]` ノート — FTS5 と埋め込みのハイブリッド検索で引き出せ、Obsidian スタイルの力学モデルグラフとして閲覧できます。各エントリはいつ有効になり何に置き換えられたかを記録し、グラフは**発効時刻で絞り込め**ます — これは現存するエントリへのフィルタであって、過去の再生ではありません。 |
| <img src="docs/assets/icons/badge-check.svg" width="20"><br/>**設計から正直に** | ガードレールが「それはもうやりました」というでっち上げの主張を捕捉し、エージェントの自己報告を実際に実行された内容へと結び付けます。**削除**が独りでに適用されることはなく、必ず受信箱に届いてあなたが承認または却下します。ホストエージェントによる**上書き**は即座に反映されますが、本文を置き換えるのではなくポインタを書くため、元のエントリは残り、ワンクリックで取り消せます。スポーンが共有メモリに対して提案するものも、同様に受信箱を通ります。 |
| <img src="docs/assets/icons/key-round.svg" width="20"><br/>**ローカルファースト、キーはあなたのもの** | あなたのマシン、あなたの API キー、そして間に挟まる**サードパーティのサーバーはゼロ**。複数のプロバイダーを設定すればそれらをまたぐルーティングを有効にできます(既定は単一モデル)。judge と router のロールは常に主モデルに固定され、評価が安いモデルへ流れることはありません。6 言語の i18n と 6 種類のテーマパレット(ライト + ダーク)を同梱。 |
...
スポーンのプロンプトは自動的に書き換えられます — ただし、あなたの目に触れる前に、ホールドアウトされた過去タスクで自らを証明しなければなりません。引き分けを除いて最低 10 組、その 60% 以上に勝ち、どの評価軸でも現行版より悪くないこと。合成タスクでの勝利で実タスクの後退を埋め合わせることはできず、回答を長くすることで勝つこともできません。不合格なら破棄され、表に出ることはありません。合格すれば、読みやすい差分を添えた提案カードが届きます。変更が反映されるのは、**あなたが Promote を押したときだけ**です。

## 発効時刻で絞り込めるセカンドブレイン

<div align="center">
  <img src="docs/assets/fig03-second-brain.png" alt="図 03 — セカンドブレイン:メモリは自動的に形成され、スポーンはハイブリッド検索で読み出し、モデルによる削除は受信箱を通り、各エントリは発効時刻を記録する" width="100%">
</div>

メモリはひとりでに形成されます — ルーターが抽出した事実と、セッション終了時の蒸留によって。そしてスポーンは、FTS5 と埋め込みのハイブリッド検索でそれを読み返します。各エントリはいつ有効になり何に置き換えられたかを記録し、Obsidian スタイルのグラフは発効時刻で絞り込めます。正確に言えば、これは**今も存在する**エントリへのフィルタであって、過去の再生ではありません — 削除やその場での編集は痕跡を残さず、エントリの終了時刻は後継から推測されるだけで記録されていません。モデルがメモリの**削除**を望むときは、提案がまず受信箱に届きます。ホストエージェントによる**上書き**は即座に反映されますが、置き換えられたエントリは残り、ワンクリックで元に戻せます。
